...
class Bee:
    def __init__(self, x, y, isCollector=False):
    ...
    def draw(self, screen):
        ...
        if self.isCollector: #сборщик
            pygame.draw.circle(screen, (255,0,0), self.getPos(), 20, 1)
            if len(self.nearestFlowers)>0:
                pygame.draw.line(screen, (255, 150, 150), self.nearestFlowers[0].getPos(), self.getPos(), 1)
            if self.attachedFlower is not None:
                flower.draw_flower(screen, self.x-20, self.y, (200, 170, 50), (255, 223, 50))
        else: #разведчик
    ...
    def behave(self, flowers):# стратегический уровень управления
        if self.isCollector: #для пчелы-сборщика
            if self.state == 0 and self.target is not None:  # поедание цветов
                self.findNearestFlowers(flowers)
                f = self.nearestFlowers[0]
                self.control(f.getPos())
                if dist(self.getPos(), f.getPos()) < 25:
                    flowers.remove(f)
                    self.attachedFlower=f
                    self.state = 1
                    b.nearestFlowers = []
            elif self.state == 1:  # перемещение к гнезду
                self.control(self.nest.getPos())
                if dist(self.getPos(), self.nest.getPos()) < 25:
                    self.state = 0
                    self.attachedFlower=None
        else:#для пчелы-разведчика
            if self.state==0 and self.target is not None: #разведка
                self.findNearestFlowers(flowers)
                self.control(self.target)
                if dist(self.getPos(), self.target)<25:
                    self.state=1
                    b.nearestFlowers = []
            elif self.state==1: #перемещение к гнезду
                self.control(self.nest.getPos())
...
if __name__ == "__main__":
    ...
    collectors=[Bee(350, 350, isCollector=True),
                Bee(450, 350, isCollector=True),
                Bee(550, 350, isCollector=True)
                ]
    for c in collectors:
        c.nest=nest
        bees.append(c)
    ...
    while running:
        ...
        #мультиагентный уровень управления
        scouts=[b for b in bees if not b.isCollector]
        if all(b.state==2 for b in scouts):
            if all(b.target is not None for b in scouts): #после разведки
                qq=[b.metric for b in scouts]
                i = np.argmax(qq)
                winner = scouts[i]
                nest.x, nest.y=winner.target
                gatherBees(scouts, nest)
            else: #после движения к гнезду
                launchBees(scouts, nest)

        collectors = [b for b in bees if b.isCollector]
        if all(b.state==2 for b in collectors):
            if all(b.target is not None for b in collectors): #после разведки
                gatherBees(collectors, nest)
            else: #после движения к гнезду
                launchBees(collectors, nest)
